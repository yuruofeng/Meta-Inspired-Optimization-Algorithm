...
export function RobustComparisonPage() {
  const { selectedIds, toggleAlgorithm, setSelectedIds, clearSelection } = useAlgorithmStore();
  const { algorithms, robustBenchmarks } = useCatalogData();
  ...
  const robustFunctionsByType = useMemo(() => {
    return robustBenchmarks.reduce((acc, func) => {
      if (!acc[func.type]) {
        acc[func.type] = [];
      }
      acc[func.type].push(func);
      return acc;
    }, {} as Record<RobustBenchmarkType, typeof robustBenchmarks>);
  }, [robustBenchmarks]);

  const selectAll = () => setSelectedIds(algorithms.map((algorithm) => algorithm.id));
  ...
  const handleRunComparison = async () => {
    ...
    const benchmark = robustBenchmarks.find(f => f.id === selectedBenchmark);
    if (!benchmark) {
  ...
  const selectedBenchmarkFunc = useMemo(() => {
    return robustBenchmarks.find(f => f.id === selectedBenchmark);
  }, [robustBenchmarks, selectedBenchmark]);
